...
const DVDLogo: React.FC<DVDLogoProps> = (props) => {
  ...
  const moveDVDLogo = () => {
    setState((prevState) => {
      const newState = {
        ...prevState,
        x: prevState.x + prevState.xSpeed,
        y: prevState.y + prevState.ySpeed,
      };
  ...
  useEffect(() => {
    const intervalId = setInterval(() => moveDVDLogo(), MS_PER_FRAME);
    return () => clearInterval(intervalId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state]);

  const { r, g, b, x, y } = state;
  return (
    <svg width={props.width} height={props.height} className={'dvd'} style={{ backgroundColor: 'black' }}>
      <g>
